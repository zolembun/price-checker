...
if df_main.empty or 'รหัสสินค้า' not in df_main.columns:
    ...
    # 👇👇👇 โค้ดนักสืบ เริ่มตรงนี้ 👇👇👇
    st.warning(f"🔍 ข้อมูลที่แอปมองเห็นจาก Google Sheet ตอนนี้:")
    if df_main.empty:
        st.code("ดึงตารางมาได้ แต่ว่างเปล่า 100% (0 บรรทัด)\n👉 เป็นไปได้ว่าแอปไปอ่านผิดแท็บ (ไม่ได้อ่านแท็บแรกสุด)")
    else:
        st.write(f"ดึงข้อมูลมาได้ทั้งหมด: {len(df_main)} บรรทัด")
        st.code(f"ชื่อหัวคอลัมน์ที่อ่านได้คือ:\n{list(df_main.columns)}")
    # 👆👆👆 โค้ดนักสืบ จบตรงนี้ 👆👆👆
